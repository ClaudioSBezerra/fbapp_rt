-- Fix RPC functions to prevent duplication from participants table
-- and ensure correct joining by trimmed cod_part
...
BEGIN
    ...
    -- Fix: Lateral join to get single participant match, handling potential duplicates and whitespace
    LEFT JOIN LATERAL (
        SELECT p2.nome, p2.cnpj
        FROM public.participantes p2 
        WHERE p2.filial_id = bd.filial_id 
          AND TRIM(p2.cod_part) = bd.cod_part
        ORDER BY p2.created_at DESC -- Prefer most recent if duplicate
        LIMIT 1
    ) p ON true
